...
This module deliberately contains presentation-only changes. The V6.2.2
widgets, database logic and navigation remain unchanged.
"""
...
QLineEdit, QComboBox, QDateEdit, QTextEdit {
    ...
    border-radius: 8px;
    padding: 8px 10px;
    selection-background-color: #0a82c8;
}
...
/* Make the editor read like a detail panel rather than a raw form. */
.card {
    background: #ffffff;
    border: 1px solid #dce4e9;
    border-radius: 10px;
...
def apply_v63_visuals(window):
    """Apply small presentation-only adjustments after MainWindow is built."""
    # Keep the version badge synchronized with the release without changing
    # any application behavior.
    for label in window.findChildren(__import__('PySide6.QtWidgets', fromlist=['QLabel']).QLabel):
        if label.objectName() == "version" or label.text().strip() == "V6.2":
            label.setText("V6.3")
            label.setObjectName("version")

    # Give the main table a slightly more spacious, readable first column.
    ...
    if table is not None:
        ...
        table.setSelectionBehavior(__import__('PySide6.QtWidgets', fromlist=['QAbstractItemView']).QAbstractItemView.SelectRows)
"""
