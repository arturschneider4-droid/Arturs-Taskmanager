...
class TaskDialog(QDialog):
    def __init__(self, parent, task=None, default_project=None):
        ...
        self.project.addItem("Kein Themengebiet", None)
        c = connect()
        for r in c.execute("SELECT id,name FROM projects ORDER BY name"):
            self.project.addItem(r["name"], r["id"])
        c.close()
        pid = task["project_id"] if task else default_project
        if pid is not None:
        ...
        for k, v in PRIORITIES.items():
            self.priority.addItem(v, k)
        if task:
            self.priority.setCurrentIndex(max(0, self.priority.findData(task["priority"])))
        self.due = QDateEdit()
        ...
        for k, v in RECURRENCE.items():
            self.recurrence.addItem(v, k)
        if task:
            self.recurrence.setCurrentIndex(max(0, self.recurrence.findData(task["recurrence"])))
        self.desc = QTextEdit(task["description"] if task else "")
        ...
        root.addWidget(QLabel("Unteraufgaben"))
        row = QHBoxLayout()
    ...
    def values(self):
        return {
            ...
            "subtasks": [
                (self.subs.item(i).text(), self.subs.item(i).checkState() == Qt.Checked)
...
class ProjectDialog(QDialog):
    def __init__(self, parent, name=""):
        super().__init__(parent)
        self.setWindowTitle("Themengebiet")
        l = QVBoxLayout(self)
        l.addWidget(QLabel("Name des Themengebiets"))
